...
- **style-id：** `knit-doll`
- **显示名：** `knit-doll`
- **别名：** 针织玩偶、毛线玩偶、羊毛毡玩偶、纤维玩偶、Cozy Knit Doll
- **适用：** 人物角色、动物吉祥物、亲子与陪伴主题、季节插图、App 活动页、头像、海报和温暖叙事场景
- **默认画布：** 竖版 `1024x1536`；方形 `2048x2048`，横版 `1536x1024`
- **默认质量：** `high`
- **文件名前缀：** `knit-doll-`
...
以下信号必须同时出现；缺少两项以上即不属于本系列：

1. 材质分区清楚：人类角色的脸与手平滑低绒，纤维集中在头发和服装；动物可使用短绒身体并搭配针织服饰。
...
- 人物角色建立明确的五层材质体系：平滑玩偶皮肤、羊毛毡或毛线头发、刷毛毛毡化针织上装、较紧密的下装面料、硬质微缩鞋与道具，至少清楚区分其中四层。动物角色按身体、服装与配饰实际需要使用，不强制添加鞋子或人类服装结构。
- 人物脸部、耳朵、颈部和手必须使用一致的平滑哑光软陶、软乙烯或极细密无绒织物；只保留轻微手工起伏，不得出现可辨毛线、粗纤维、针织纹或毛毡毛面。
- 人物角色的可见纤维主要集中于头发和服装，通常占人物可见面积的 45–70%；不得为了达到纤维感把皮肤也毛绒化。
- 头发使用排列清楚的成束毛线、小型卷曲纤维、编结线圈或整块针刺羊毛表达。用户未指定发型时，默认使用整齐的小簇卷发或分层毛线发片；禁止自动生成粗绳脏辫、章鱼触手状发束或凌乱绳结。
- 毛衣、开衫、围巾、帽子和外套使用“针织结构＋毛毡化刷毛表层”：底层仍能看到尺度明确的线圈与罗纹，表面覆盖均匀柔软的短纤维和少量自然飞毛，边缘形成温暖发光的绒毛光晕。
- 上装不得像刚织好的干净棉线、硬挺钩针、塑料绳或过度清晰的规则线圈；针脚之间要有轻微毛绒融合和手工张力差异。
- 裤子、短裤与裙装默认使用细密梭织毛毡、微缩灯芯绒、丹宁或低绒紧密针织，纹理尺度比上装更细、更平整；不能与毛衣使用同样粗大的线圈。
- 鞋子必须使用微缩帆布、磨砂橡胶、哑光皮革、麂皮或致密无纺毡，具有独立鞋底、鞋头、鞋带或搭扣结构；禁止毛线编织鞋、钩针鞋、毛衣材质鞋底和整双羊毛鞋。
...
- **人物全身：** 头部约占全身高度的 22–32%，颈部偏细长，肩窄，四肢简化，鞋子略大，形成略显笨拙的定格动画玩偶比例。
- **人物头像或半身：** 头部略大，脸型圆润，耳朵与鼻子可适度夸张；头发和衣领形成清楚的纤维轮廓。
- **动物角色：** 使用卵形或梨形身体、短粗肢体和稍大的头；默认穿至少一件针织服装或配饰，除非用户明确要求自然状态。
- **人形角色：** 默认穿至少一件明确的针织单品，例如毛衣、开衫、围巾或针织帽；不要让人物赤裸或只穿无纹理衣物。
- 手部保持连指玩偶结构或 3–4 根简化圆指；仅在握持动作需要时增加可读手指，不刻画指甲、关节和皮肤纹理。
- 保留一点人工制作的不完美：左右耳或线圈略有差异、轮廓轻微歪斜，但不得破损、脏乱或粗制滥造。
- 避免正面证件照式站姿。默认让躯干轻微三分之四转向、头部倾斜约 5–12°、肩线和肘部高低不同，并通过轻微重心偏移形成自然动作。
- 双手分别持有道具时，把两个道具安排在不同高度和不同前后距离；不得让双臂水平镜像展开或像产品展示架一样对称。

## 3. 五官与情绪

- 人物眼睛优先使用小型黑色珠眼或竖椭圆黑点；动物可使用略大的黑色纽扣眼，但不得包含复杂虹膜和写实眼白。
- 珠眼允许一个极小而克制的棚灯反光点；禁止多重星芒、高亮玻璃球和动漫水光眼。
- 鼻子使用一个小圆体、短锥体或轻微凸起；嘴巴使用短弧或简单开口。
- 大笑、惊喜等强情绪只在用户动作需要时使用；嘴内部保持图形化，最多出现一排简化牙齿和柔软舌形。
- 表情必须与动作联动：大笑时默认使用闭合弯月眼或明显眯眼、头部轻微后仰和图形化张嘴；不能一边大笑一边保持正视圆珠眼和僵硬头位。
- 使用暖粉或珊瑚粉的圆形腮红、小量雀斑和轻微眉线增加亲和力；不使用真实妆容、法令纹或皮肤毛孔。
- 表情要真诚、温暖、略显稚拙，避免性感、写实、阴森玩偶或僵硬假笑。
- 用户只说“圆框眼镜”而未指定材质时，默认使用略夸张、偏厚、哑光深棕或炭灰色玩偶镜框；不要自动使用纤细写实金属丝镜框。

## 4. 针织穿搭语言

- 服装以宽松、层叠、略微 oversized 的轮廓为主：厚毛衣、开衫、围巾、针织帽、短裤、阔腿裤和大鞋。
- 一套造型使用 1 个主针织单品、1–2 个辅助服装层和最多 2 个小配饰；完整人形角色在用户未指定服装时，默认设计为“内搭＋刷毛针织外层＋结构化下装＋硬质鞋”，避免只穿一件孤立毛衣。
- 通过粗细不同的线圈、罗纹方向、毛线蓬松度和色块分层区分衣物，而不是依赖复杂印花。
- 允许条纹、简单小花或色块拼接；禁止密集图案、真实品牌标志、可识别商标和未经要求的文字。
- 帽子、围巾、衣领和袖口要有厚实柔软的体积，不能像贴在身体上的平面纹理。
- 主针织单品必须具有可见的柔软毛毡感：短密绒毛覆盖约一半的清晰线圈边缘，但不能完全抹掉针织结构。
- 下装默认选择炭灰、燕麦或丹宁色的细密梭织、灯芯绒或低绒毡；不得默认使用和上装同样的粗针织材质。
- 鞋子默认选择带白色或奶油色橡胶鞋底的微缩帆布运动鞋、磨砂皮短靴或圆头皮鞋，允许清楚鞋带和拼片，但不出现品牌 Logo。
- 袜子可以是细针织；鞋子本体和鞋底始终不得是针织或松散毛线。
- 每套完整造型加入 3–6 个小而清楚的工艺细节，例如大纽扣、罗纹领口、贴袋、抽绳、可见明线、帽顶毛球、围巾流苏、织物贴布或包带；这些细节使用同一色板，不得出现随机文字和品牌。
- 只允许一个主要图案语言：宽条纹、稀疏小花、撞色色块或素色纹理四选一；不得同时堆叠多套花纹。
- 可加入一个非叙事性的织物贴布、毛毡徽章或小挂饰作为视觉焦点，但不得自动增加新动物伙伴、人物或可读文字。

## 5. 配色系统

- 默认使用明亮、轻快、梦幻的高明度中高彩度色盘，整幅图使用 5–7 个协调颜色，并同时包含暖色与冷色。
- 核心色库：晴空蓝、水蓝、薄荷绿、嫩草绿、奶油白、柠檬黄、芥末黄、桃粉、珊瑚粉、杏橙和焦糖色。
- 背景使用 3–4 个大面积环境色，主体服装使用 2–3 个呼应或对比色，深色只用于眼睛、头发、鞋底和必要轮廓。
- 背景与最大服装色块必须在冷暖或明度上形成清楚分离；中性米色与棕色面积合计不超过画面约 25%，肤色除外。
...
- **人物：** 保留用户指定的年龄、性别、发型、动作和职业信号，同时统一为手工玩偶比例与针织穿搭。
- **动物：** 保留耳朵、喙、尾巴、鼻子等物种信号；默认采用短绒身体与一件针织服饰。
- **亲子或拥抱：** 使用重叠的大弧形手臂与柔软服装体块表现包裹感，确保每个角色的脸仍可辨认。
- **宠物组合：** 用明显大小差和相互朝向建立关系；宠物也必须是纤维玩偶，不得混入写实动物。
- **手机、眼镜与包：** 简化为圆角微缩道具，不生成品牌 Logo、界面文字或精密工业细节；圆框眼镜默认使用偏厚哑光玩偶镜框。
- **鞋子：** 使用微缩帆布、橡胶、磨砂皮、麂皮或致密无纺毡，保留鞋底、鞋头、鞋带或搭扣；不得做成毛线编织或毛绒袜套。
- **精确文字：** 仅在用户明确提供时出现；优先做成织物贴布、刺绣或毛毡字，逐字照抄。
...
- 人物脸、耳、颈部或手出现明显毛线、针织纹、粗纤维和毛毡绒面，失去平滑玩偶皮肤与纤维服装的材质分区。
- 人物头发在未指定时变成粗绳脏辫、触手状发束或杂乱绳结。
- 针织纹只是平面贴图，缺少可见线圈、罗纹、粗纺厚度和自然飞毛。
- 上装线圈过于干净、干硬、规则，缺少覆盖在针织结构上的短绒毛毡感与柔软光晕。
- 裤子与毛衣使用完全相同的粗针织材质，或整套服装没有粗细、软硬和绒毛密度差异。
- 鞋面、鞋底或鞋带由松散毛线和针织线圈组成；鞋子没有帆布、磨砂皮、橡胶等清楚硬质层。
- 人形角色没有衣服，或没有至少一件清楚可辨的针织单品。
...
```text
Textile correction only: keep the subject, pose, colors and composition unchanged. On upper garments, preserve readable knitted loops and ribbing but overlay them with dense short brushed wool fibers, a soft felted mohair halo and slight handmade stitch-tension variation. Keep lower garments finer and flatter. Keep the human face, ears, neck and hands smooth matte and low-fuzz. Do not turn the entire outfit into one uniform yarn material.
```

人物比例或服装失败时，仅追加：

```text
Character correction only: preserve identity and action. Rebuild the figure as a handmade stop-motion textile doll with a larger simplified head, slender neck, simplified limbs, slightly oversized shoes and at least one clearly knitted oversized garment. Keep the complete figure and all requested props visible. Do not add brands or accessories.
```

五官过度写实或眼睛过亮时，仅追加：

```text
Face correction only: replace realistic eyes and facial anatomy with simple small black bead or short-oval eyes, one tiny sculpted nose, one short curved mouth and soft round blush. Remove complex iris, sclera, multiple highlights, realistic lips, pores and makeup. Keep expression, pose, hair, wardrobe and background unchanged.
```
...
```text
Fiber grooming correction only: keep every material and color unchanged, but shorten and align the flyaway fibers, reveal the silhouette, facial features, clothing edges and hand positions, and preserve a soft tactile halo without tangled or dirty fuzz.
```
...
```text
Color and backdrop correction only: preserve subject identity, pose, materials, required props and the selected background preset. Replace muddy, dark, beige or sepia colors with a bright high-key dreamlike palette of 5–7 coordinated colors selected from sky blue, aqua, mint, fresh grass green, warm cream, lemon yellow, peach pink, coral and apricot orange. Keep 3–4 environment colors and 2–3 subject colors with clear warm-cool separation and luminous daylight.
```
...
```text
Craft-detail correction only: keep identity, pose, palette and required props unchanged. Preserve the hero fuzzy felted-knit garment and add one supporting clothing layer plus 3–6 coherent handmade details selected from ribbed collar or cuffs, oversized buttons, patch pocket, drawstrings, visible topstitching, pompom, scarf fringe, fabric applique or a simple strap. Use only one pattern language and do not add text, brands, new characters or unrelated accessories.
```

上装缺少毛毡绒感或全身材质相同时，仅追加：

```text
Wardrobe material hierarchy correction only: keep identity, pose, colors and composition unchanged. Make the sweater or cardigan a brushed felted knit with readable loops beneath dense short fuzzy wool fibers and a soft edge halo. Change trousers to finer woven felt, corduroy, denim or low-nap fabric. Preserve smooth skin and organized felted hair. Do not use the same chunky knit material on every garment.
```

鞋子被生成为毛线编织时，仅追加：

```text
Footwear correction only: keep the entire character and composition unchanged. Replace knitted, crocheted or furry footwear with miniature non-knit shoes made from canvas, suede or matte leather, with a molded cream rubber sole, clear toe cap, simple laces or straps, subtle stitching and no brand logo. Socks may remain fine knit, but the shoe upper and sole must contain no yarn loops.
```

姿态僵硬或表情与动作不匹配时，仅追加：

```text
Pose and expression correction only: keep the complete character, wardrobe, colors and required props. Turn the torso slightly three-quarter, tilt the head 5–12 degrees, offset the shoulders and elbows, shift the body weight, and stagger the two hand-held props at different heights and depths. For laughter, close or strongly squint the eyes, tilt the head slightly back and keep one simple graphic open mouth. Avoid mirrored arms and mannequin symmetry.
```
